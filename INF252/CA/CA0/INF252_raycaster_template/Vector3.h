...
public:
    ...
    /// Vector sum
	template <class U>
    const Vector3<data_t> operator+(const Vector3<U>& other) const {
        Vector3<data_t> vecNew(*this);
        vecNew += other;
        return vecNew;
    }
	...
	template <class U>
    const Vector3<data_t> operator*(const U& f) const {
		Vector3<data_t> vecNew(*this);
		vecNew *= f;
		return vecNew;
	}
    ...
    const Vector3<data_t> operator-() const {
...
const Vector3<V> operator*(const U& f, const Vector3<V>& vec) {
    return vec.operator *(f);
}
